...
load('/home/claudia/variabilidad/boundaries.Rdata')
...
## Si utilizase datos mensuales, también tendría que hacer las medias por meses de la temperatura.
...
fooParallel <- function(data, data2, filename='YearlyProductivity30_fixed_temp', modeTrk = 'fixed', nodes=detectCores(), blocks=6,...){
    ...
    fooProd <- function(g0){
        ...
        Prod <- prodGCPV(lat= lat,
                         dataRad= list(G0dm=g0[2:365], Ta=g0[366:730]),
...
##writeRaster(yProdTwo, filename='YearlyProductivity30_two2.grd', overwrite=TRUE)
